package dev.stone.practice.util.uuid;

import lombok.Getter;
import dev.stone.practice.Phantom;
import dev.stone.practice.util.uuid.bukkit.BukkitUUIDCache;
import dev.stone.practice.util.uuid.listener.UUIDListener;
...
public final class UUIDCache implements IUUIDCache {
    ...
    public UUIDCache() {
        ...
        Phantom.getInstance().getServer().getPluginManager().registerEvents(new UUIDListener(), Phantom.getInstance());
    ...
    public void monitor(String message) {
        MONITOR_CACHE.keySet().forEach(uuid -> {
            ...
            final Player player = Phantom.getInstance().getServer().getPlayer(uuid);

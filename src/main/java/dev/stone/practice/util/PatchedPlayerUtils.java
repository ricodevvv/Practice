...
public class PatchedPlayerUtils {
    ...
    public void denyMovement(Player player) {
        if (player.hasMetadata("noDenyMove")) {
            player.removeMetadata("noDenyMove", Phantom.getInstance());
            return;
        }
        ...
        player.setMetadata("denyMove", new FixedMetadataValue(Phantom.getInstance(), true));
    ...
    public void allowMovement(Player player) {
        ...
        player.removeMetadata("denyMove", Phantom.getInstance());
    }

    public static List<String> mapToNames(Collection<UUID> uuids) {
        return uuids.stream().map(Phantom.getInstance().uuidCache::name).collect(Collectors.toList());
    }

    public static String getFormattedName(UUID uuid) {
        return Phantom.getInstance().getUuidCache().name(uuid);
    }
}
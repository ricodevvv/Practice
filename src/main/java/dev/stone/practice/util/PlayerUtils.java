...
public final class PlayerUtils {
    ...
    public static void animateDeath(Player player, boolean hideAfter) {
        ...
        try {
            ...
            if (hideAfter) {
                Phantom.getInstance().getServer().getScheduler().runTaskLater(Phantom.getInstance(), () -> {
    ...
    public static void animateDeath(Player player, Player watcher) {
        ...
        try {
            ...
            Phantom.getInstance().getServer().getScheduler().runTaskLater(Phantom.getInstance(), () -> ((CraftPlayer) watcher).getHandle().playerConnection.sendPacket(new PacketPlayOutEntityDestroy(entityId)), 40L);
        } catch (Exception ex) {
            ex.printStackTrace();
        }

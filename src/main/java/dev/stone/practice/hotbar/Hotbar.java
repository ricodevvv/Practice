package dev.stone.practice.hotbar;
...
public class Hotbar {
    ...
    private static EdenItem loadItem(String path) {
        BasicConfigurationFile file = Phantom.getInstance().getHotbar();
        try {
            ItemStack itemStack = new ItemBuilder(Material.valueOf(file.getString(path + ".material")))
                    .durability(file.getInteger(path + ".durability"))
                    .name(file.getString(path + ".name"))
                    .lore(file.getStringList(path + ".lore"))
                    .build();
            return new EdenItem(file.getBoolean(path + ".enabled"), itemStack, file.getInteger(path + ".slot"), file.getString(path + ".command"));
        } catch (Exception e) {
            return INVALID;
        }
    }
